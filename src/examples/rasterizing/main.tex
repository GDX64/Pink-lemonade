...
\end{equation}
\begin{equation}
P = \frac{1}{w}\sum_{i=1}^{N} w_i\left[P_i + (\mu_i-\mu)(\mu_i-\mu)^T\right].
\end{equation}
...
\begin{equation}
\begin{aligned}
P_{ij} = \frac{w_i}{w_i + w_j}P_i + \frac{w_j}{w_i + w_j}P_j + \frac{w_i w_j}{(w_i + w_j)^2}(\mu_i-\mu_j)(\mu_i-\mu_j)^T.
\end{aligned}
\end{equation}
\endgroup
...
For each merged kernel, we keep its center $\mu\in\mathbb{R}^2$, weight $w$, and covariance $P\in\mathbb{R}^{2\times 2}$. In the vertex shader, we build a Cholesky factor $L$ (as in Gaussian sampling methods \cite{Bishop2006}) such that
\begin{equation}
P = LL^T,
\end{equation}
with
...
\end{equation}
Starting from standardized quad coordinates $z\in[-3,3]^2$, the shader computes
\begin{equation}
x = \mu + sLz,
\end{equation}
...
The fragment shader does not need to invert $P$ per pixel. Instead, it receives $z$ as a varying from the vertex stage, and rasterization hardware linearly interpolates $z$ across the two triangles of the quad. Therefore each fragment already has the local standardized coordinates needed for the quadratic form
\begin{equation}
d_M^2 = z^Tz,
\end{equation}
which is equivalent to the Mahalanobis distance in screen coordinates:
\begin{equation}
d_M^2 = (x-\mu)^T (s^2P)^{-1}(x-\mu).
\end{equation}
The kernel value is then evaluated as
\begin{equation}
K(x)=\exp\left(-\frac{1}{2}d_M^2\right),
\end{equation}
...
\begin{figure}
  ...
  \caption{Two-component Gaussian merge example for equal-variance kernels. At centroid separation $\rho=\lVert\Delta\rVert/\sigma=1$, the moment-preserving single-Gaussian approximation induces a low integrated squared error, consistent with the analytical result in Section~\ref{sec:discussion}.}
