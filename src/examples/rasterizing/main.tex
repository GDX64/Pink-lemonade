...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{End-to-end rendering pipeline. Visible samples are clipped and merged on the CPU, then submitted to a three-pass GPU pipeline. In Pass 1, the vertex shader expands one instanced screen-aligned quad per merged kernel, and the fragment shader performs KDE accumulation via quad rasterization with additive blending into an HDR texture. Pass 2 is a compute shader that performs global-maximum reduction. Pass 3 performs dynamic color mapping. The reduction output is consumed both by the color-mapping shader (same frame) and by asynchronous CPU readback for legend scaling.}
