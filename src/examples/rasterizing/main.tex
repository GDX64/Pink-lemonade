...
The visible points are passed through a \textit{weighted merge} downsampler before GPU upload. The algorithm scans points left to right, maintaining an open cluster. When a new point arrives, it computes the hypothetical weighted centroid that would result from absorbing the point into the cluster. If both the earliest cluster point and the incoming point remain within a configurable screen-space threshold of that centroid, the point is absorbed and the centroid is updated. Otherwise the cluster is flushed as a single output point and a new cluster begins.

The centroid of each output point carries the summed weight of all its constituents, conserving the total signal: a merged cluster renders with a proportionally brighter splat than any individual constituent would. This weight-conserving merge is analogous in spirit to Gaussian mixture reduction techniques \cite{4383588}, which similarly collapse components while preserving aggregate statistics, though our criterion is spatial proximity in screen space rather than a KL-divergence bound.
...
This section formalizes the merge operation used by our downsampler and relates it to the KL-based Gaussian mixture reduction criterion of Runnalls \cite{4383588}.
...
These equations preserve zeroth, first, and second moments of the merged pair, so total weight, weighted mean, and covariance are conserved locally.

Runnalls derives a pair-selection score from an upper bound on KL discrimination after a merge:
\begin{equation}
B(i,j)=\frac{1}{2}\Big[(w_i+w_j)\log\det(P_{ij})-w_i\log\det(P_i)-w_j\log\det(P_j)\Big].
\end{equation}
Minimizing $B(i,j)$ prefers merges that combine low-weight components, nearby means, and similar covariances, while avoiding pathological behavior of earlier criteria \cite{4383588}.

Our renderer uses a linear-time screen-space merge for interactive performance, but it follows the same principles. In practice, kernels share the same base radius, so $P_i \approx P_j \approx \sigma^2 I$ before merging, and the only growth term in $P_{ij}$ is the outer product of the centroid displacement. Therefore, keeping points close in screen space keeps covariance inflation low, which is consistent with the KL-bound objective while remaining fast enough to run on every pan/zoom update.
...
To isolate the effect of merging (rather than tonemapping), MSE is computed on the raw linear HDR accumulation buffer before opacity thresholding and colormap mapping. In this setting, each merged cluster replaces multiple nearby kernels by one weighted representative; therefore the local error grows with centroid displacement and decreases with larger kernel bandwidth.

In practice, the merge threshold $\tau$ induces the expected trade-off: increasing $\tau$ reduces the number of submitted kernels and improves frame time, but increases MSE due to stronger spatial averaging. We therefore treat $\tau$ as a quality knob and select the largest value that keeps NRMSE below a target tolerance for the current viewport.

Because the merge is recomputed in screen space after each pan/zoom update, the error is also view-dependent. This is desirable: when zoomed out, multiple nearby events become perceptually indistinguishable and can be merged with low MSE impact; when zoomed in, clusters naturally split and the approximation converges to the full KDE.
...
\end{equation}
In this two-Gaussian setting, $\Psi(\rho)$ is bounded with $\Psi(0)=0$ and $\lim_{\rho\to\infty}\Psi(\rho)=1$, consistent with ISD-style bounded similarity/error behavior discussed by Williams \cite{article}.

This gives an analytical error curve versus distance-in-sigmas. For a heatmap region of area $|\Omega|$ in chart coordinates, a practical approximation is
\begin{equation}
\mathrm{MSE}(\rho) \approx \frac{\mathrm{ISE}(\rho)}{|\Omega|},
\end{equation}
or, on a discrete grid, replacing $|\Omega|$ by the number of pixels.

For small separations ($\rho \ll 1$), $\Psi(\rho)$ grows from zero with low-order polynomial behavior, explaining why nearby kernels can be merged with little error; for large separations, the error rises quickly, which justifies capping the merge threshold in sigma units.
...
\end{equation}
which is small in practice and supports using a merge threshold on the order of $\sigma$ when prioritizing low approximation error.
