...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[stage, below=of inst] (acc) {Pass 1: Accumulation (additive KDE into HDR density texture)};
    \node[stage, below=of acc] (red) {Pass 2: Reduction (parallel max to compute global $\max(F)$)};
  ...
  \end{tikzpicture}
  \caption{End-to-end rendering pipeline. Visible samples are clipped and merged on the CPU, then submitted to a three-pass GPU pipeline: additive KDE accumulation in HDR, global-maximum reduction, and perceptual tonemapping. The reduction output is consumed both by the tonemapping shader (same frame) and by asynchronous CPU readback for legend scaling.}
